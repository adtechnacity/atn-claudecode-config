...
This audit uses `feature-dev` agents to augment manual review:

| Agent | Phase | Purpose |
|-------|-------|---------|
| `code-explorer` | Performance | Map hot paths and execution flows before analysis |
| `code-reviewer` | Security, Maintainability, Reliability | Review with confidence scoring (≥80 threshold) |

Spawn agents via the Task tool with `subagent_type` set to `feature-dev:code-explorer` or `feature-dev:code-reviewer`.
...
Check for outdated dependencies and security advisories using the appropriate package manager:
...
### 2.1 Spawn Code Reviewer (Security Focus)
...
### 2.2 Manual Security Checks
Items the agent may miss (check based on project type):
...
### 3.1 Map Hot Paths with Code Explorer
...
### 3.2 Analyze Identified Hot Paths
