...
- **`/ship`** (Phase 1) - Mandatory test audit before production release
- **`/commit`** (Step 2) - Tests must pass before committing

Related commands:
- **`/deps`** - Manage test dependencies
- **`/review`** - May identify missing test coverage during code review

Related agents:
- **`typescript-code-reviewer`** - Reviews test quality for TypeScript projects

## Build System Detection

Detect the project's build system and run appropriate test commands:

| Indicator | Build System | Test Command | Coverage |
|-----------|--------------|--------------|----------|
| `package.json` | npm/Node.js | `npm test` | `npm run test:coverage` |
| `mix.exs` | Elixir/Mix | `mix test` | `mix test --cover` |
| `Cargo.toml` | Rust/Cargo | `cargo test` | `cargo tarpaulin` |
...
| Agent | Phase | Purpose |
|-------|-------|---------|
| `feature-dev:code-reviewer` | Audit Existing Tests | Identify redundant or low-quality tests |
| `feature-dev:code-explorer` | Identify Critical Paths | Find untested business logic and hot paths |
| `typescript-code-reviewer` | TypeScript Projects | Senior-level test quality review |
...
For TypeScript projects, also consider using `typescript-code-reviewer` agent for deeper analysis.
...
- Quality over quantity - fewer well-designed tests beat many redundant ones
- Tests are part of `/commit` validation - they must pass
- Tests are mandatory before `/ship` - no shipping with failing tests
