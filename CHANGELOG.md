...
- **`hooks/format-and-lint.sh`**: Respect the project's declared formatter. The format phase now walks up from the edited file to detect `oxfmt`/`biome`/`dprint` configs and runs that formatter, falling back to `prettier` only when the project declares prettier or no formatter at all. Prevents prettier from reflowing files against the project's real config (e.g. oxfmt `printWidth` 100 vs prettier's 80). Binary lookup walks up to find `node_modules/.bin`, so it works in git worktrees and hoisted monorepos; a declared-but-missing formatter is skipped rather than overwritten with prettier.

### Added

- **`.gitignore`**: Ignore credentials (`.credentials.json`) and runtime state (`daemon/`, `jobs/`, `security/`, `daemon.log`, `gh-pr-status-cache.json`, `.last-cleanup`, `.last-update-result.json`).
